...
p.1 <- ggplot(df.1, aes(x = sample.1, y = ..density..)) +
    geom_histogram(bins = 20, fill = "#00a2ed", alpha = 0.8) +
...
dist.5 <-
    0.1 * dnorm(sample.5, 1, sd = 0.1) + 0.7 * dnorm(sample.5, 2, sd = 0.5) + 0.2 * dnorm(sample.5, 5, sd = 1)

df.5 <- data.frame(sample.5, dist.5)

p.5 <- ggplot(df.5, aes(x = sample.5, y = ..density..)) +
    geom_histogram(bins = 50, fill = "#00a2ed", alpha = 0.8) +
