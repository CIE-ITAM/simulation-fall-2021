...
Como la Tau de Kendall entre X e Y es ed 0.55 y la función 1/X es monótona decreciente en todo el dominio de la v.a X (ya que X>0 por hipótesis), tenemos que la Tau de Kendall para (1/X,Y) es de -0.55. Ahora, aplicando el mismo resultado que ahora, tenemos que la v.a Y es positiva en todo su soporte y por lo tanto, la transformación 1/Y es monótona decreciente en todo el dominio de Y. Por lo tanto, la Tau de Kendall para la pareja (1/X,1/Y) es de -(-0.55)=0.55.
...
empCopula<-function(u,v,xvec,yvec){
  ...
  for (i in 1:length(xvec)){
    ...
    if(rango_x[i]/(length(xvec)+1)<=u & rango_y[i]/(length(xvec)+1)<=v){
      vect_res[i]=1
    }else{vect_res[i]=0}
  }
